...
# 这个矩阵后面只作为“相对瓶子中心的平移偏置”使用。
# 目标抓取姿态不再由这里的旋转决定，而是在 make_bottle_pose_in_base()
...
class PiperVisionController:
    ...
    def make_bottle_pose_in_base(self, bottle_xyz_cam):
        """
        将相机坐标系下瓶子 3D 点转换为基座坐标系下的目标抓取位姿。

        YOLO + 深度图只能稳定得到瓶子中心点，不能像 ArUco 一样直接得到瓶子的
        完整姿态。因此这里的处理方式是：

        1. 位置：
           用 T_base_ee @ T_ee_cam @ T_cam_bottle_point
           把瓶子中心从相机坐标系转换到基坐标系。
        ...
        # 生成目标抓取位姿：位置取瓶子中心，姿态只保证夹爪 X 轴朝向基坐标系 -Z。
        T_base_bottle = np.eye(4)
        T_base_bottle[0:3, 3] = T_base_bottle_point[0:3, 3]
        T_base_bottle[0:3, 0:3] = build_level_grasp_rotation(
    ...
    def get_bottle_pose(self, target_label=TARGET_BOTTLE_LABEL, max_frames=60):
        ...
        for _ in range(max_frames):
            ...
            if best_candidate is not None and _ >= 10:
                ...
                grasp_x_base = c["T_base_bottle"][0:3, 0]
